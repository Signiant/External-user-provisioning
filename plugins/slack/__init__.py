...
def removeUser(email,configMap): #removes user as a member of dev-signiant
    ...
    try:
        ...
        log = 'Slack: Remove from slack error: user does not existed or is already inactive\n error: '+ str(error) +'\n'
        instruction = 'Remove from slack error: user does not existed or is already inactive. Exception caught: '+ str(error)
